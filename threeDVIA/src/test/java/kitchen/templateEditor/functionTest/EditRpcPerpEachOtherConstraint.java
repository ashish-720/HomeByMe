...
@Test
public class EditRpcPerpEachOtherConstraint extends Browser {

	public void teEditRpcPerpToEachOtherConstraint() throws IOException {
		...
		constraints.perpendicularToEachOtherRpC(-105,2, 15,2).goToDesignPage().saveProject().close();
		TransferFile transferFile=PageFactory.initElements(driver, TransferFile.class);
		transferFile.transferFile("EditRpcPerpEachOtherConstraint");
		start.startNow().verifyFloorplan().goToDesignPage().reopenProject("EditRpcPerpEachOtherConstraint");
		constraints.editPerpToEachOtherRpc(15,2,-105,2).goToDesignPage().saveProject().close();
		transferFile.transferFile("EditRpcPerpEachOtherConstraintV1");
